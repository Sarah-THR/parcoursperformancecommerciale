...
{
    [HttpGet("contractsnexlease/{commercial}"), AllowAnonymous]
    public class GetContractNexleaseByUserNameEndpoint : Endpoint<ContractsRequest, IEnumerable<ContractSaleResponse>>
    {
        private readonly IMapper _mapper;
        private readonly IMediator _mediator;

        public GetContractNexleaseByUserNameEndpoint(IMapper mapper, IMediator mediator)
        {
            _mapper = mapper;
            _mediator = mediator;
        }
        ...
        public override async Task HandleAsync(ContractsRequest req, CancellationToken ct)
        {
            try
            {
                var result = _mapper.Map<IEnumerable<ContractSaleResponse>>(await _mediator.Send(new GetContractNexleaseByUserNameRequest
                {
